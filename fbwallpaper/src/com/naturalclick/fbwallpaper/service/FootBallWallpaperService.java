package com.naturalclick.fbwallpaper.service;
...
public class FootBallWallpaperService extends GLWallpaperService {
	...
	public FootBallWallpaperService() { 
		super(); 
	...
	private void copyVideoToCard() {
		...
		try {
			int assetIndex = MyApp.config().getInt("videoassetnumber", 0);
			if (assetIndex > videoAsset.length) {
				assetIndex = videoAsset.length - 1; 
			}
			in = assetManager.open(folder+"/"+videoAsset[assetIndex]);
	...
	public Engine onCreateEngine() {
		...
		NativeCalls.loadVideo("file:/"+"sdcard/"
				+FootBallWallpaperService.videoName);
	...
	class VideoEngine extends GLEngine {
		...
		VideoEngine() { 
			...
			renderer = new VideoRenderer(FootBallWallpaperService.this, 
					this);
